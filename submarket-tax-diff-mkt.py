...
avtot_data = pd.read_csv('./processed_data/cleaned_combined_fullval_by_year.csv')
...
for submarket, zip_codes in submarkets.items():
    ...
    if submarket_avtot.empty:
        print(f"No FULLVAL data found for Submarket {submarket} with ZIPs {zip_codes}")
        continue
...
results.to_csv('submarket_mkt_price_tax_difference.csv', index=False)
